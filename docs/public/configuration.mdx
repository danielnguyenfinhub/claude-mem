...
| `CLAUDE_MEM_COMPACT_JSON`     | _(off)_                         | Set to `true` or `1` to strip indentation from JSON MCP tool responses. Whitespace only — the parsed payload is identical. Measured at ~27% of an observation-list response, and unlike the tool manifest it is paid on every call, not once per connect. Set it on the worker too (or restart the worker after setting it), since the worker builds some responses itself. |
